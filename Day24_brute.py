...
numIntersections = 0
found = False
for delta in range(1, 99):
#    print("Delta: ", delta)
    for gamma in range(delta+1, 100):
        ...
        for i, a in enumerate(data):
            ...
            for j, b in enumerate(data):
                if i == j:
                    continue
                pos1 = add_v3v3(a[0], mul_v3_fl(a[1], delta))
                pos2 = add_v3v3(b[0], mul_v3_fl(b[1], gamma))
                vec = [x/(gamma-delta) for x in sub_v3v3(pos2, pos1)]
                startPos = sub_v3v3(pos1, vec)
                resLine = startPos, vec

                looksGood = True
                for k, test in enumerate(data):
                    if k != i and k != j:
                        intersection = findIntersection3(resLine, test)
                        if intersection is None:
                            looksGood = False
                            break
                if looksGood:
                    found = True
                    print("Looks good! ", resLine)
                    break
exit(0)
#done till delta=7  (6, 100)
